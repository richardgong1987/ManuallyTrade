...
The strategy is implemented; `ManuallyTrade.cs` is the Robot lifecycle shell that wires the
...
dotnet build "ManuallyTrade.sln"          # Debug
dotnet build "ManuallyTrade.sln" -c Release
```

A successful build produces a `.algo` package under
`ManuallyTrade/bin/<Config>/net6.0/`. The `.algo` file is the deployable
