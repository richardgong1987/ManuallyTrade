...
namespace Pdhpdl.Tests.Orders {
    public class PdhpdlOrderPlannerTests {
        ...
        public void sizes_short_to_lose_exactly_the_risk_budget_at_the_stop() {
            PdhpdlOrderPlanner planner = CreatePlanner();

            PdhpdlOrderPlanModel plan = planner.CreatePlan(ShortRequest(entry: 100.0, stopLoss: 102.0, takeProfit: 96.0), accountEquity: 10000.0);

            Assert.True(plan.IsValid);
            Assert.Equal(PdhpdlTradeDirectionModel.Short, plan.DirectionModel);
            Assert.Equal("ManuallyTrade_Short1", plan.Label);
        ...
        public void rounds_volume_down_so_a_stop_out_never_exceeds_the_budget() {
            ...
            PdhpdlOrderPlanModel plan = planner.CreatePlan(ShortRequest(entry: 100.0, stopLoss: 102.15, takeProfit: 96.0), accountEquity: 10000.0);
        ...
        public void sizes_in_account_currency_when_pip_value_differs_from_pip_size() {
            ...
            PdhpdlOrderPlanner planner = CreatePlanner(pipValue: 0.0855);
        ...
        public void sizes_long_with_stop_below_entry() {
            ...
            request.Direction = PdhpdlTradeDirectionModel.Long;

            PdhpdlOrderPlanModel plan = planner.CreatePlan(request, accountEquity: 10000.0);
        ...
        public void rejects_when_any_price_is_missing(double entry, double stopLoss, double takeProfit) {
            ...
            PdhpdlOrderPlanModel plan = planner.CreatePlan(ShortRequest(entry, stopLoss, takeProfit), accountEquity: 10000.0);
        ...
        public void rejects_short_unless_stop_is_above_entry_and_take_profit_below(double stopLoss, double takeProfit) {
            ...
            PdhpdlOrderPlanModel plan = planner.CreatePlan(ShortRequest(entry: 100.0, stopLoss, takeProfit), accountEquity: 10000.0);
        ...
        public void rejects_when_risk_percentage_is_not_positive(double riskPct) {
            ...
            PdhpdlOrderPlanModel plan = planner.CreatePlan(ShortRequest(entry: 100.0, stopLoss: 102.0, takeProfit: 96.0, riskPct), accountEquity: 10000.0);
        ...
        public void rejects_when_sized_volume_is_below_broker_minimum() {
            PdhpdlOrderPlanner planner = CreatePlanner(volumeInUnitsMin: 100.0);
        ...
        public void rejects_when_sized_volume_is_above_broker_maximum() {
            PdhpdlOrderPlanner planner = CreatePlanner(volumeInUnitsMax: 10.0);
        ...
        private static PdhpdlOrderPlanner CreatePlanner(double pipValue = 0.1, double volumeInUnitsMin = 1.0,
            double volumeInUnitsMax = 1_000_000.0) {
            return new PdhpdlOrderPlanner(new FakeSymbolModel {
                PipSize = 0.1,
                LotSize = 100.0,
