...
namespace Pdhpdl.Tests.Orders {
    // Checks the lot size PdhpdlOrderPlanner computes for realistic broker symbol settings.
    ...
    public class PositionSizingTests {
        ...
        public void xauusd_one_percent_with_a_10_dollar_stop_is_a_tenth_of_a_lot() {
            PdhpdlOrderPlanner planner = new(Xauusd(pipValuePerUnit: 0.01));
            ...
            PdhpdlOrderPlanModel plan = planner.CreatePlan(Short(entry: 2400.0, stopLoss: 2410.0, takeProfit: 2380.0, riskPct: 1.0),
        ...
        public void xauusd_rounds_down_to_the_unit_step_so_the_loss_stays_under_budget() {
            ...
            PdhpdlOrderPlanModel plan = planner.CreatePlan(Short(entry: 2350.50, stopLoss: 2358.00, takeProfit: 2330.0, riskPct: 2.0),
        ...
        public void eurusd_one_percent_with_a_50_pip_stop_is_two_tenths_of_a_lot() {
            PdhpdlOrderPlanner planner = new(Eurusd());

            // riskMoney = 10000 × 1% = 100 USD. Stop 1.0900 − 1.0850 = 50 pips.
            // One unit loses 50 × 0.0001 = 0.005 USD, so 100 / 0.005 = 20000 units = 0.20 lot.
            PdhpdlOrderPlanModel plan = planner.CreatePlan(Short(entry: 1.0850, stopLoss: 1.0900, takeProfit: 1.0750, riskPct: 1.0),
                accountEquity: 10000.0);
        ...
        public void eurusd_rounds_down_to_the_one_thousand_unit_step() {
            ...
            PdhpdlOrderPlanModel plan = planner.CreatePlan(Short(entry: 1.0850, stopLoss: 1.0880, takeProfit: 1.0790, riskPct: 1.0),
                accountEquity: 10000.0);
        ...
        public void eur_account_on_usd_quoted_gold_sizes_in_account_currency() {
            ...
            PdhpdlOrderPlanner planner = new(Xauusd(pipValuePerUnit: 0.0092));
            ...
            PdhpdlOrderPlanModel plan = planner.CreatePlan(Short(entry: 2400.0, stopLoss: 2405.0, takeProfit: 2390.0, riskPct: 1.0),
                accountEquity: 10000.0);
